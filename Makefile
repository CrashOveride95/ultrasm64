...
SRC_DIRS += src src/boot src/game src/engine src/ff src/audio src/menu src/buffers actors levels bin data assets asm lib sound
...
ALL_DIRS := $(BUILD_DIR) $(addprefix $(BUILD_DIR)/,$(SRC_DIRS) finalfs $(GODDARD_SRC_DIRS) $(LIBZ_SRC_DIRS) $(ULTRA_BIN_DIRS) $(BIN_DIRS) $(TEXTURE_DIRS) $(TEXT_DIRS) $(SOUND_SAMPLE_DIRS) $(addprefix levels/,$(LEVEL_DIRS)) rsp include) $(YAY0_DIR) $(addprefix $(YAY0_DIR)/,$(VERSION)) $(SOUND_BIN_DIR) $(SOUND_BIN_DIR)/sequences/$(VERSION) \
  $(addprefix $(BUILD_DIR)/finalfs/, actors levels bin data assets sound $(BIN_DIRS) $(addprefix levels/,$(LEVEL_DIRS))) $(YAY0_DIR) $(addprefix $(YAY0_DIR)/,$(VERSION)) $(SOUND_BIN_DIR)
...
$(ELF): $(O_FILES) $(YAY0_OBJ_FILES) $(SEG_FILES) $(BUILD_DIR)/$(LD_SCRIPT) undefined_syms.txt $(SOUND_FILES) $(LIBZRULE) $(GODDARDRULE)
	...
	$(V)$(LD) --gc-sections -L $(BUILD_DIR) -T undefined_syms.txt -T $(BUILD_DIR)/$(LD_SCRIPT) -Map $(BUILD_DIR)/sm64.$(VERSION).map --no-check-sections $(addprefix -R ,$(SEG_FILES)) -o $@ $(O_FILES) -L$(LIBS_DIR) -l$(ULTRALIB) -Llib -Llib/gcclib/$(LIBGCCDIR) -lgcc -lrtc -lnustd -lcart -lhvqm2 $(LIBZLINK) $(GODDARDLINK) -u sprintf -u osMapTLB
...
ifeq      ($(CONSOLE),n64)
	...
	$(V)$(N64CKSUM) $@
	$(V)cp $(BUILD_DIR)/sm64.$(VERSION).map $(BUILD_DIR)/finalfs/sm64.$(VERSION).map
	$(V)cp $@ $(subst $(BUILD_DIR),$(BUILD_DIR)/finalfs,$@)
